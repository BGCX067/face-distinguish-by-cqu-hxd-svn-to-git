...
BEGIN_MESSAGE_MAP(CFolderTreeCtrl, CTreeCtrl)
	...
	ON_WM_LBUTTONDOWN() //左键 打开+
	ON_WM_RBUTTONDOWN()//右键 弹出菜单
	//}}AFX_MSG_MAP
	ON_UPDATE_COMMAND_UI(IDM_EVENT_OPEN,MenuEventOpen)  //右键菜单 资源管理器打开路径
	ON_UPDATE_COMMAND_UI(IDM_EVENT_SELECTALL,MenuEventSelectAll) //右键菜单 全选
	ON_UPDATE_COMMAND_UI(IDM_EVENT_UNSELECTALL,MenuEventUnSelectAll) //右键菜单 全反选
...
void CFolderTreeCtrl::Init()
{
	...
	m_imageListFolder.Add(AfxGetApp()->LoadIcon(IDI_FOLDER_CLOSE));//AfxGetApp( )这个函数可以得到当前引用的指针CWinApp*，通过这个指针可以访问到这个进程中的对象。
...
void CFolderTreeCtrl::AddDrive(char drivePathName[])
{
	...
	if(GetVolumeName(drivePathName, volumeName))//待修改问题：总是返回0，于是 不显示卷标对应的的名字?
...
//选中了某个节点后，查看他的父节点下的所有子节点是不是都被选中了
...
void CFolderTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(uFlags & TVHT_ONITEMSTATEICON)//如果选中的是选择框位置
	{
		if(GetCheck(hItem))
	...
	Expand(hItem, TVE_TOGGLE);//TVE_TOGGLE 如果列表当前是展开的则收缩列表；反之则展开列表。 
}
...
int CFolderTreeCtrl::IsValidFolder(char folderPathName[])
{
	...
	------------------------------------------------------------------*/
	CFileFind fileFind;
	if(fileFind.FindFile(folderPathName)){
		fileFind.FindNextFile();
		if(fileFind.IsDirectory()){
			//if(folderPathName[0]=='E'){
...
void CFolderTreeCtrl::SelectFolder(char folderPathName[])
{
	...
	{	
		...
		if(folderName[1]==':'){
			char volumeName[100];
			char tempFolderName[100];
			strcpy(tempFolderName,folderName);
			strcat(tempFolderName,"\\\0");
			this->GetVolumeName(tempFolderName,volumeName);
			if(volumeName[0]!=0)
			strcat(strcat(strcat(folderName," [\0"),volumeName),"]\0");
		}
		//---------------------------------------------------------------
...
HTREEITEM CFolderTreeCtrl::InsertChildFolder(char childFolderName[], HTREEITEM parentItem)
{
	...
	while(pCurrentItem)
	{
		GetFolderPathName(pCurrentItem,parentFolderPathName);
		if(strcmp(childFolderName, GetItemText(pCurrentItem).GetBuffer(100)) == 0)
		//if(strcmp(childFolderName,parentFolderPathName) == 0)
			return pCurrentItem;
